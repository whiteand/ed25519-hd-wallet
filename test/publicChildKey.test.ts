...
describe('publicChildKey', () => {
  it('private 0', () => {
    ...
    const res = publicChildKey([root[1], root[2]], 0);
    if (!res) {
      throw new Error('expected result');
    }
    expect(hex(res[0])).toMatchInlineSnapshot(
  ...
  it('private 1', () => {
    const root = rootKeySlip10(seed);
    const res = publicChildKey([root[1], root[2]], 1);
    if (!res) {
      throw new Error('expected result');
    }
  ...
  it('private 2', () => {
    const root = rootKeySlip10(seed);
    const res = publicChildKey([root[1], root[2]], 2);
    if (!res) {
      throw new Error('expected result');
    }
  ...
  it('private 42', () => {
    const root = rootKeySlip10(seed);
    const res = publicChildKey([root[1], root[2]], 42);
    if (!res) {
      throw new Error('expected result');
    }
  ...
  it('private 42/1', () => {
    ...
    expect(hex(res2[0])).toMatchInlineSnapshot(
